...
public final class Complex {
	...
	public Complex multiply(Complex c) {
		return new Complex((re() * c.real) - (im() * c.imag), (im() * c.real) + (re() * c.imag));
	}
	...
	@Override
	public int hashCode() {
		double valR = this.real * 9999;
		double valI = this.imag * 99;
		double valA = valR + valI;
		int all = (int) valA;
		return all;
	...
	public boolean equals(Object obj) {
		...
		if ((this.real == that.real) && (this.imag == that.imag)) {
			return true;
		} else {
			return false;
		}
	...
	public static Complex valueOf(String s) {
		...
		if (parts.size() != 3) {
			throw new IllegalArgumentException("parts does not have 3 elements");
		}

		if (!(parts.get(2).endsWith("i"))) {
			throw new IllegalArgumentException("third element does not end with i");
		}
		String u = parts.get(1);
		String k = "-";
		if (u.equals(k)) {
			double first = Double.valueOf(parts.get(0));
			String a = parts.get(2);
			String z = a.replace("i", "");
			double third = Double.valueOf(z);
