...
public class LZWDictionary {
	...
	public LZWDictionary(String characters) {
		...
		{
			String p = Character.toString(characters.charAt(i));
			if(!map.containsKey(p) && !list.contains(p))
			{
				map.put(p, ind++);
				list.add(p);
			}
	...
	public void reset() {
		...
		List<String> t = list.subList(index, list.size());
	...
	public int add(String entry) {
		if(list.contains(entry) && map.containsKey(entry))
		{
	...
	public boolean contains(String s) {
		if(list.contains(s) && map.containsKey(s))
		{
			return true;
		}
		else
