...
public class LZWCompressor {
	...
	public List<Integer> encode() {
		...
		String w = Character.toString(this.input.charAt(0));
		for(int i = 1; i < this.input.length(); i++)
		{
			String c = Character.toString(this.input.charAt(i));
			String wc = w + c;
			...
			{
				int p = this.dict.indexOf(w);
				li.add(p);
				this.dict.add(wc);
				w = c;
			}
		}
		li.add(this.dict.indexOf(w));
		this.dict.reset();
		return li;
	}
	...
	public String decode(List<Integer> encoded) {
		...
		result = "" + this.dict.get(prev);
		int i = 1;
		while(i < encoded.size())
		{
			String s = this.dict.get(prev);
			...
			result = result + s;
			String entry = this.dict.get(prev) + Character.toString(s.charAt(0));
			this.dict.add(entry);
			prev = next;
			i++;
		}
	...
	public double compressionRatio() {
		double d = this.input.length();
		double c = encode().size();
		double r = d / c;
		return r;
	}
